...
BEGIN
  ...
  IF column_name = 'auth_email' OR column_name = 'auth.email' THEN
    query := format(
  ...
  ELSE
    ...
    -- Check if column name contains dot (e.g. table.col)
    -- Ideally we should handle relations but for now assume direct column if simple name
    -- or remove table prefix if present and matches table_name
