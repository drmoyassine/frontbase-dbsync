...
const CHAT_COMPLETIONS_PARAMS: SchemaParam[] = [
...
const EMBEDDINGS_PARAMS: SchemaParam[] = [
...
const IMAGE_GEN_PARAMS: SchemaParam[] = [
    { name: 'model', type: 'string', required: true, defaultVal: '—', defaultRaw: null, description: 'Model slug' },
...
const AUDIO_TRANSCRIPTION_PARAMS: SchemaParam[] = [
...
const AUDIO_SPEECH_PARAMS: SchemaParam[] = [
    { name: 'model', type: 'string', required: true, defaultVal: '—', defaultRaw: null, description: 'Model slug' },
...
const RESPONSES_PARAMS: SchemaParam[] = [
    { name: 'model', type: 'string', required: true, defaultVal: '—', defaultRaw: null, description: 'Model slug (e.g. gpt-oss-120b)' },
...
const MODALITY_MAP: Record<string, ModalityConfig> = {
    llm: {
    ...
    embedder: {
        ...
        baseBody: (m) => ({ model: m, input: 'Hello, world!' }),
    },
    'text-embeddings': {
        endpoint: '/v1/embeddings',
        params: EMBEDDINGS_PARAMS,
    ...
    image_gen: {
        ...
        params: IMAGE_GEN_PARAMS,
        baseBody: (m) => ({ model: m, prompt: 'A serene mountain lake at sunset' }),
    },
    'text-to-image': {
        endpoint: '/v1/images/generations',
        params: IMAGE_GEN_PARAMS,
    ...
    stt: {
        ...
        params: AUDIO_TRANSCRIPTION_PARAMS,
        baseBody: (m) => ({ model: m, file: '<base64-encoded-audio>' }),
    },
    'speech-recognition': {
        endpoint: '/v1/audio/transcriptions',
        params: AUDIO_TRANSCRIPTION_PARAMS,
    ...
    tts: {
        ...
        baseBody: (m) => ({ model: m, input: 'Hello, this is a test of text to speech.' }),
    },
    responses: {
...
function getContextSize(modelId?: string): string | null {
    if (!modelId) return null;
    return CONTEXT_HINTS[modelId] ?? null;
}

function getModelCardUrl(modelId?: string): string | null {
    if (!modelId) return null;
