...
# =============================================================================
# Turso Database Settings (Edge State DB)
# =============================================================================

class TursoSettings(BaseModel):
    turso_enabled: bool = False
    turso_url: Optional[str] = None      # libsql://your-db.turso.io
    turso_token: Optional[str] = None    # auth token
...
@router.get("/turso/", response_model=TursoSettings)
async def get_turso_settings():
    """Get Turso database settings for the Edge state DB."""
    settings = load_settings()
    turso = settings.get("turso", {})
    return TursoSettings(
        turso_enabled=turso.get("turso_enabled", False),
        turso_url=turso.get("turso_url"),
        turso_token=turso.get("turso_token"),
    )


@router.put("/turso/", response_model=TursoSettings)
async def update_turso_settings(settings_update: TursoSettings):
    """
    Update Turso database settings.
    
    When enabled with valid credentials, the publish strategy will 
    automatically use Turso instead of local HTTP POST.
    """
    settings = load_settings()
...
@router.post("/turso/test/", response_model=TursoTestResult)
async def test_turso_connection(settings_update: TursoSettings):
    """Test Turso database connection with provided credentials."""
    if not settings_update.turso_url or not settings_update.turso_token:
        return TursoTestResult(success=False, message="URL and Token are required")
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                return TursoTestResult(success=True, message="Connection successful!")
            else:
        ...
        return TursoTestResult(success=False, message=str(e))
