...
async def publish_draft_to_engine(
    ...
    if bool(engine.is_shared) or bool(engine.is_system):
        ...
        if project and not bool(project.is_default):
            raise HTTPException(
                status_code=403,
                detail="Secondary projects cannot publish workflows to the shared community engine. Please assign a dedicated Edge Engine to this project."
            )
