...
async def list_tenants(
    ...
    for t in tenants:
        ...
        result.append(TenantResponse(
            ...
            slug=str(t.slug),
            name=str(t.name),
            plan=str(t.plan) if t.plan else "free",
            status=str(t.status) if t.status else "active",
...
async def get_tenant(
    ...
    return {
        "tenant": {
            ...
            "plan": str(tenant.plan) if tenant.plan else "free",
            "status": str(tenant.status) if tenant.status else "active",
