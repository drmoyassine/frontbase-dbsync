 ...
 * and builds the package). The product pins that SHA in FRAMEWORK_PIN — the
 * mirror image of the framework's CONSOLE_PIN, which pins the product SHA the
 * console is vendored from. Bumping the pin changes what self-host builds
...
const pinPath = path.join(root, 'FRAMEWORK_PIN');
...
console.log(`FRAMEWORK_PIN -> ${commit}`);
